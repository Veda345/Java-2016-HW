...
public class Implementor implements Impler, JarImpler {
    ...
    private void classConstr(Class clazz) throws ImplerException {
        ...
        for (Constructor c : clazz.getDeclaredConstructors()) {
            if (Modifier.isPrivate(c.getModifiers()))
                continue;
            f = false;
            strB.append("\t").append(getModifiers(c.getModifiers()).replace("abstract ", ""));
            strB.append(clazz.getSimpleName()).append("Impl(").append(getParameters(c.getParameterTypes())).append(")");
            ...
            strB.append("{ ");
            strB.append("super(").append(getParameterNames(c.getParameterTypes())).append(");");
            strB.append("}\n");
        }
    ...
    private void descriptionClass(Class clazz) throws ImplerException {
        ...
        if (clazz.getPackage() != null) {
            strB.append("package ").append(clazz.getPackage().getName()).append(";\n");
        }

        classMod(clazz);
        strB.append("class ").append(clazz.getSimpleName()).append("Impl ");

            if (!clazz.isInterface()) {
                strB.append("extends ").append(clazz.getSimpleName()).append(" ");
            } else {
                strB.append("implements ").append(clazz.getSimpleName()).append(" ");
            }
    ...
    private void methods(Class clazz){
        ...
        for (Method m : methods) {
            ...
            for (Annotation a : annotations) {
                strB.append("@").append(a.annotationType().getName()).append(" ");
            }
            strB.append("\n");
            ...
            if (!m.getReturnType().isPrimitive()) {
                strB.append(m.getReturnType().getCanonicalName()).append(" ").append(m.getName());
            } else {
                strB.append(getType(m.getReturnType())).append(" ").append(m.getName());
            }

            strB.append("(").append(getParameters(m.getParameterTypes())).append(") ");
            ...
            if (mReturnType == void.class) {
                ...
                if (mReturnType.isPrimitive()) {
                    if (mReturnType == boolean.class) {
                        ob = false;
                    } else {
                        ob = 0;
                    }
                    strB.append(ob.toString()).append(";\n\t}\n");
                } else {
                    strB.append("null;\n\t}\n");
                }
            }
